...
const EmailTemplateVisualEditor = ({ template, onSave, onCancel }) => {
  ...
  const [content, setContent] = useState({
    ...
    body: template?.body ? (isHtmlContent(template.body) ? htmlToPlainText(template.body) : template.body) : "",
  ...
  const generatePreview = () => {
    ...
    const html = `
      ...
      <body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: ${styling.backgroundColor};">
        <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background-color: ${styling.backgroundColor}; min-height: 100%;">
          <tr>
            <td align="center" style="padding: 40px 20px;">
              <table cellpadding="0" cellspacing="0" border="0" width="600" style="max-width: 600px; background-color: ${styling.containerBackgroundColor}; border-radius: ${styling.containerCorners === 'ROUNDED' ? '12px' : '0'};">
                
                <!-- Header -->
                <tr>
                  <td style="padding: 40px 30px; text-align: ${styling.businessNamePosition.toLowerCase()}; border-bottom: 1px solid #e0e0e0;">
                    ${styling.logoUrl ? `
                      <div style="text-align: ${styling.logoPosition.toLowerCase()}; margin-bottom: ${styling.showBusinessName ? '15px' : '0'};">
                        <img src="${styling.logoUrl}" alt="Logo" style="max-width: ${getLogoSize(styling.logoSize)}; height: auto;" />
                      </div>
                    ` : ''}
                ...
                <tr>
                  <td style="padding: 40px 30px; text-align: ${styling.textAlignment.toLowerCase()};">
                    <div style="color: ${styling.textColor}; font-size: 16px; line-height: 1.6;">
                      ${formatTextForPreview(replaceVariables(content.body, previewData))}
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="padding: 0 30px 40px 30px; text-align: ${styling.buttonAlignment.toLowerCase()};">
                    <a href="${previewData.privateFeedbackUrl}" style="display: inline-block; background-color: ${styling.buttonColor}; color: #ffffff; padding: 16px 40px; text-decoration: none; border-radius: ${styling.buttonStyle === 'PILL' ? '50px' : '8px'}; font-weight: bold; font-size: 18px;">
                      ${styling.buttonText}
                    </a>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="background-color: #f8f9fa; padding: 25px 30px; border-top: 1px solid #e0e0e0; text-align: center;">
                    <p style="margin: 0 0 5px 0; font-size: 14px; color: #666;">${previewData.businessPhone}</p>
                    <p style="margin: 0 0 15px 0; font-size: 14px; color: #666;">${previewData.businessWebsite}</p>
                    <p style="margin: 0; font-size: 12px; color: #999;">
                      <a href="${previewData.unsubscribeUrl}" style="color: #666; text-decoration: none;">Unsubscribe</a>
                    </p>
                  </td>
                </tr>
            ...
            <div className="space-y-6">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <p className="text-sm text-blue-800">
                  💡 <strong>Tip:</strong> Your styling settings are shared across all templates. 
                  Set your brand once in the STYLING tab, then focus on your message here.
                </p>
              </div>
...
Thanks!"
                ...
                <p className="mt-2 text-sm text-gray-600">
                  💡 <strong>Tip:</strong> Just type normally - we'll make it look beautiful automatically! Use variables like {'{{customerName}}'} for personalization.
                </p>
              </div>
            ...
            <div className="space-y-6">
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-6">
                <p className="text-sm text-purple-800">
                  🎨 <strong>Brand Styling:</strong> These settings apply to all your email templates. 
                  Set your brand once and it'll be consistent across all campaigns.
                </p>
              </div>

              {/* Logo & Business Name */}
              <div className="border-b border-gray-200 pb-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">LOGO & BUSINESS NAME</h3>
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div className="col-span-3">
                    ...
                    <input
                      ...
                      onChange={(e) => setStyling(prev => ({ ...prev, logoUrl: e.target.value }))}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {["SMALL", "MEDIUM", "LARGE"].map((size) => (
                        <button
                          key={size}
                          type="button"
                          onClick={() => setStyling(prev => ({ ...prev, logoSize: size }))}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {["LEFT", "CENTER", "RIGHT"].map((pos) => (
                        <button
                          key={pos}
                          type="button"
                          onClick={() => setStyling(prev => ({ ...prev, logoPosition: pos }))}
                          className={`px-3 py-2 text-xs border rounded-lg ${
                ...
                <div className="flex items-center mb-2">
                  <input
                    ...
                    onChange={(e) => setStyling(prev => ({ ...prev, showBusinessName: e.target.checked }))}
                    className="mr-2"
                  />
                  <label htmlFor="showBusinessName" className="text-sm font-medium text-gray-700">
                    Show Business Name
                  </label>
                </div>
              </div>
              ...
              {/* Custom Image */}
              <div className="border-b border-gray-200 pb-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">IMAGE OPTIONS</h3>
                ...
                <div className="flex items-center mb-4">
                  <input
                    ...
                    onChange={(e) => setStyling(prev => ({ ...prev, showCustomImage: e.target.checked }))}
                    className="mr-2"
                  />
                  <label htmlFor="showCustomImage" className="text-sm font-medium text-gray-700">
                    Show Custom Image
                  </label>
                </div>
                ...
                {styling.showCustomImage && (
                  <input
                    ...
                    onChange={(e) => setStyling(prev => ({ ...prev, customImageUrl: e.target.value }))}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg"
              ...
              {/* Text Alignment */}
              <div className="border-b border-gray-200 pb-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">TEXT ALIGNMENT</h3>
                <div className="grid grid-cols-3 gap-2">
                  {["LEFT", "CENTER", "RIGHT"].map((align) => (
                    <button
                      key={align}
                      type="button"
                      onClick={() => setStyling(prev => ({ ...prev, textAlignment: align }))}
              ...
              <div className="border-b border-gray-200 pb-6">
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) => setStyling(prev => ({ ...prev, buttonText: e.target.value }))}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                  />
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {["ROUNDED", "PILL"].map((style) => (
                        <button
                          key={style}
                          type="button"
                          onClick={() => setStyling(prev => ({ ...prev, buttonStyle: style }))}
                          className={`px-3 py-2 text-xs border ${style === "ROUNDED" ? "rounded-lg" : "rounded-full"} ${
                    ...
                    <div className="grid grid-cols-3 gap-1">
                      {["LEFT", "CENTER", "RIGHT"].map((align) => (
                        <button
                          key={align}
                          type="button"
                          onClick={() => setStyling(prev => ({ ...prev, buttonAlignment: align }))}
              ...
              {/* Container Corners */}
              <div className="pb-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">CONTAINER CORNERS</h3>
                <div className="grid grid-cols-2 gap-2">
                  {["ROUNDED", "SHARP"].map((corner) => (
                    <button
                      key={corner}
                      type="button"
                      onClick={() => setStyling(prev => ({ ...prev, containerCorners: corner }))}
                      className={`px-4 py-3 border ${corner === "ROUNDED" ? "rounded-lg" : "rounded-none"} ${
        ...
        <div className="w-1/2 bg-gray-50 overflow-y-auto p-6">
          <div className="sticky top-0 bg-gray-50 pb-4 mb-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600">See how your email will look to customers</p>
          </div>
