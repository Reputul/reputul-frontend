...
const WorkflowCard = ({ workflow, onWorkflowUpdate, userToken }) => {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:8080/api/automation/workflows/${workflow.id}/status`,
  ...
  return (
    <div className={`bg-white/10 backdrop-blur-xl rounded-2xl border transition-all duration-300 hover:shadow-lg hover:scale-[1.02] p-6 ${
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <h3 className="text-white font-bold text-lg truncate">{workflow.name}</h3>
          </div>
          <p className="text-blue-200 text-sm leading-relaxed">{workflow.description}</p>
        </div>
        ...
        <div className="bg-white/5 rounded-xl p-3 border border-white/10">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-white font-medium">
              {workflow.triggerConfig.delay_hours && `${workflow.triggerConfig.delay_hours}h delay`}
              {workflow.triggerConfig.delay_days && `${workflow.triggerConfig.delay_days}d delay`}
              {!workflow.triggerConfig.delay_hours && !workflow.triggerConfig.delay_days && 'Immediate'}
            </span>
          </div>
        </div>
      )}
