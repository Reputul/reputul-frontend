...
const CustomerManagementPage = () => {
  ...
  const handleAddCustomer = useCallback(async () => {
    if (!newCustomer.businessId || !newCustomer.name || !newCustomer.email || !newCustomer.serviceDate || !newCustomer.serviceType) {
  ...
  const filteredCustomers = customers.filter(customer => {
    const businessMatch = selectedBusiness === 'all' || customer.business?.id.toString() === selectedBusiness;
  ...
  const getTagColor = (tag) => {
    switch(tag) {
      case 'REPEAT_CUSTOMER': return 'bg-blue-100 text-blue-800';
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-200">
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-600 mb-1">{customer.business?.name}</p>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 mt-1">Manage customers and send targeted review requests</p>
            </div>
            <button
            ...
            <div className="flex items-center">
              <div className="p-2 bg-purple-100 rounded-lg mr-3">
                <svg className="w-6 h-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2m-9 4V4m0 2h10m0-2V4m-5 4v6m-3-3h6" />
                </svg>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-600">Repeat Customers</p>
                <p className="text-2xl font-bold text-gray-900">
                  {customers.filter(c => c.tags && c.tags.includes('REPEAT_CUSTOMER')).length}
                </p>
              </div>
            </div>
          </div>
        ...
        {filteredCustomers.length === 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">No customers match your current search criteria.</p>
            <button
              onClick={() => {
                setSearchTerm('');
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-900">{showCustomerDetails.name}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600">Business</label>
                  <p className="text-lg text-gray-900">{showCustomerDetails.business?.name}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600">Email</label>
                  <p className="text-lg text-gray-900">{showCustomerDetails.email}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600">Phone</label>
                  <p className="text-lg text-gray-900">{showCustomerDetails.phone}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600">Service Date</label>
                  <p className="text-lg text-gray-900">{showCustomerDetails.serviceDate}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600">Status</label>
                  <span className={`inline-block px-3 py-1 text-sm font-medium rounded-full ${getStatusColor(showCustomerDetails.status)}`}>
              ...
              <div>
                ...
                <p className="text-lg text-gray-900">{showCustomerDetails.serviceType}</p>
              </div>

              <div>
                <label className="text-sm font-medium text-gray-600">Tags</label>
                <div className="flex flex-wrap gap-2 mt-2">
                  {showCustomerDetails.tags && showCustomerDetails.tags.map((tag) => (
                    <span
                      key={tag}
              ...
              {showCustomerDetails.notes && (
                <div>
                  ...
                  <p className="text-gray-900 bg-gray-50 p-3 rounded-lg mt-2">{showCustomerDetails.notes}</p>
                </div>
              )}
