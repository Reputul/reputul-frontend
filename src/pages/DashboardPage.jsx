...
const DashboardPage = () => {
  ...
  const handleUpdateBusiness = useCallback(async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:8080/api/businesses/${editingBusiness}`, editBusinessData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleRequestReviews = useCallback(async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:8080/api/reviews/request", requestReviewsData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteBusiness = useCallback(async (businessId) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/businesses/${businessId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const metrics = {
    ...
    averageRating: businesses.length > 0 
      ? (businesses.reduce((sum, biz) => sum + (biz.reputationScore || 0), 0) / businesses.length).toFixed(1)
      : '0.0',
    responseTime: 18,
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-all duration-200">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className={`text-3xl font-bold ${color === 'yellow' ? 'text-yellow-500' : color === 'green' ? 'text-green-600' : color === 'purple' ? 'text-purple-600' : 'text-blue-600'}`}>
              {value}
            </p>
  ...
  const ReviewForm = React.memo(({ businessId }) => {
    ...
    return (
      <div id={`review-form-${businessId}`} className="bg-gray-50 rounded-lg p-4 mb-4">
        <h5 className="text-sm font-semibold text-gray-900 mb-3">Add Manual Review</h5>
          ...
          <div className="flex justify-between items-start mb-4">
            <div className="flex-1">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <span className={`px-3 py-1 text-xs font-semibold rounded-full ${getBadgeColor(business.badge)}`}>
                  {business.badge}
                </span>
              </div>
            ...
            <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V6a2 2 0 00-2-2h-4M14 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2" />
                </svg>
              </button>
              <button 
          ...
          <div className="grid grid-cols-3 gap-6 mb-6">
            <div className="text-center p-4 bg-gradient-to-br from-yellow-50 to-orange-50 rounded-xl border border-yellow-200">
              <div className="flex items-center justify-center mb-2">
                <svg className="w-6 h-6 text-yellow-500 fill-current mr-2" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                </svg>
                <span className="text-2xl font-bold text-yellow-600">
                  {summary ? summary.averageRating.toFixed(1) : business.reputationScore || '0.0'}
                </span>
              </div>
            ...
            <div className="text-center p-4 bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl border border-purple-200">
              ...
              <p className="text-sm font-medium text-purple-700">{business.badge || 'Unranked'}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button 
              onClick={() => {
                ...
                const reviewForm = document.querySelector(`#review-form-${business.id}`);
                if (reviewForm) {
  ...
  const RecentActivity = () => {
    const allReviews = Object.values(reviewsMap).flat().sort((a, b) => 
      new Date(b.createdAt) - new Date(a.createdAt)
    ).slice(0, 3);

    // Mock data for demonstration when no real reviews
    const mockActivity = [
      { type: 'review', user: 'John D.', business: "Joe's Plumbing", rating: 5, time: '2 hours ago', comment: 'Excellent service!' },
      { type: 'response', user: 'You', business: 'Elite Auto Repair', time: '4 hours ago', action: 'replied to review' },
      { type: 'review', user: 'Sarah M.', business: 'Downtown Hair Salon', rating: 4, time: '1 day ago', comment: 'Great experience' }
    ];

    const activityData = allReviews.length > 0 ? allReviews.map(review => {
                  ...
                  <p className="text-sm text-gray-900">
                    ...
                    {activity.type === 'review' ? (
                      <>
                        ...
                        {activity.comment && (
                          <span className="block text-gray-600 italic mt-1">"{activity.comment}"</span>
                        )}
                      </>
                    ) : (
                      <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <p className="text-gray-600 mt-1">Manage your business reputation with confidence</p>
            </div>
            <button 
              onClick={() => setShowAddBusiness(true)}
          ...
          <div className="lg:col-span-2">
            ...
            {businesses.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
                ...
                <p className="text-gray-600 mb-8 max-w-md mx-auto">Get started by adding your first business to manage its reputation and start collecting reviews.</p>
                <button 
                  onClick={() => setShowAddBusiness(true)}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border border-green-200 p-6">
              ...
              <p className="text-sm text-green-800 leading-relaxed">
                Businesses that respond to reviews within 24 hours see a 15% increase in customer satisfaction and rank higher in local search results.
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-xl">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-blue-900">{metrics.totalReviews}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-xl">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-900">{metrics.averageRating}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-4 rounded-xl">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-purple-900">{metrics.totalBusinesses}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 p-4 rounded-xl">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm text-yellow-600 font-medium">Response Time</p>
                    <p className="text-2xl font-bold text-yellow-900">{metrics.responseTime}h</p>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    {businesses.map((business) => {
                      ...
                      return (
                        <tr key={business.id} className="border-b border-gray-100">
                          ...
                          <td className="py-3">{summary ? summary.totalReviews : business.reviewCount || 0}</td>
                          <td className="py-3">
                            <div className="flex items-center space-x-1">
                              <span className="text-yellow-500">★</span>
                              <span>{summary ? summary.averageRating.toFixed(1) : business.reputationScore || '0.0'}</span>
                            </div>
                          </td>
                          <td className="py-3">
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                              business.badge === 'Top Rated' ? 'bg-green-100 text-green-800' : 
                              business.badge === 'Rising Star' ? 'bg-yellow-100 text-yellow-800' :
