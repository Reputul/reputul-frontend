...
const AutomationPage = () => {
  ...
  const fetchAutomationData = async () => {
    try {
      ...
      // Fetch workflows
      const workflowsResponse = await axios.get('http://localhost:8080/api/automation/workflows', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setWorkflows(workflowsResponse.data);

      // Fetch execution metrics
      const metricsResponse = await axios.get('http://localhost:8080/api/automation/monitoring/metrics?hoursBack=168', {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      const activityResponse = await axios.get('http://localhost:8080/api/automation/monitoring/executions?hoursBack=24&limit=10', {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            <div>
              ...
              <p className="text-blue-200">Manage your customer review automation workflows</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 p-6 h-full">
              <h3 className="text-xl font-bold text-white mb-4">Recent Activity</h3>
              
              {recentActivity.length > 0 ? (
                <div className="space-y-3">
                  {recentActivity.slice(0, 8).map((activity, index) => (
                    <div key={activity.id || index} className="flex items-center space-x-3 p-3 bg-white/5 rounded-xl">
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-blue-300 text-xs mt-1">Automation events will appear here</p>
                </div>
              )}
            </div>
          </div>
